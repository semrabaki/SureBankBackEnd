...
@Service
public class UserService { //User i databasee kaydetmek icin olusturdk
	...
	public void registerUser(RegisterRequest registerRequest) {
        ...
        if(userRepository.existsByEmail(registerRequest.getEmail())) {
			...
			throw new ConflictException(String.format(ExceptionMessage.EMAIL_ALREADY_EXIST_MESSAGE, registerRequest.getUserName()));
		}
        
       if(userRepository.existsBySsn(registerRequest.getSsn())) {
			
			throw new ConflictException(String.format(ExceptionMessage.SSN_ALREADY_EXIST_MESSAGE, registerRequest.getUserName()));
		}
       
       Set<Role>userRoles=new HashSet<>();
     
       //We add ROLE_CUSTOMER fir every registred user by default
	...
	//this methods returns the user information who called this method
	...
	private Set<Role> addRoles(Set<String> userRoles){ //this method convert the string into role class
		Set<Role> roles=new HashSet<>();
		
		if(userRoles==null) {
			Role role=roleRepository.findByName(UserRole.ROLE_CUSTOMER)  //we create default role 
